...
def main():
    ...
    with open('./dataloaders/activity_labels.txt', 'r') as f:
        class_names = f.readlines()
        f.close()
    # init model
    model = C3D_model.C3D(num_classes=3)
    checkpoint = torch.load('run/run_0/models/C3D-ucf101_epoch-4.pth.tar', map_location=lambda storage, loc: storage)
